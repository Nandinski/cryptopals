...
// ConvertHexTo64 converts hex string to base64
...
// FixedXOR does the xor between two strings
...
// XOROverByteVector performs XOR over 2 byte arrays of the same length
func XOROverByteVector(b1, b2 []byte) ([]byte, error) {
	if len(b1) != len(b2) {
		return b1, errors.New("Can't perform XOR on different sized arrays")
...
// SingleKeyXORDecipher Breaks single byte XOR on hexString
...
// DetectSingleCharacterXORInFile -
...
// EncodeWithRepeatingXOR Implement repeating-key XOR
...
// BreakRepeatingXORCypher -
// First guesses the top 3 lengths of the key
// Breakes Single Byte XOR for each byte cyphered with the same key
	...
	for _, keySize := range likelyKeySize {
		...
		for col := 0; col < keySize; col++ {
			for row := 0; row < len(column)-1; row++ {
				if row*keySize+col >= len(cypheredText) {
					continue
				} // Ignore it if you can't read it
