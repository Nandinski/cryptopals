...
func (i InvalidCharacter) Error() string{
	...
	return fmt.Sprintf("Invalid character - not of base64: %s", string(i))
}
...
func Decode(dst, src []byte) (n int, err error) {
	...
	for i = 0; i < len(src)/4; i++ {
		if ok := convertB64Block(dst[n:], src[srcI:]); ok {
			...
			var dstInc int
			dstInc, err = checkForQuantum(dst[n:], src[srcI:])
			srcI += 4
			n += dstInc
...
func checkForQuantum(dst, src []byte) (n int, err error){
	...
	for i := 0; i < len(dstBytes); i++ {
		...
		if(srcI == len(src)){
			return 0, InvalidCharacter(src[srcI-1])
		}
		...
		if c != StdPadding {
			...
			return 0, InvalidCharacter(src[srcI])
		}
